 <?php
...

  {
    ...
    public function caller_data()
    {
    		$this->db->select('master_admin.*,caller_admin.*');
			$this->db->from('master_admin');
			$this->db->join('caller_admin', 'master_admin.admin_user_id = caller_admin.caller_admin_id', 'inner');
			$query = $this->db->get();
    ...
    public function get_call_filter_data($table,$where1,$item1,$where2,$item2)
    {
    	$this->db->select('*');
      	$this->db->from($table);
      	if(!empty($item1))
        {
	   	$this->db->where($where1,$item1);
        }
    ...
    public function get_call_filter_data_sub($table,$where1,$item1,$where2,$item2,$where3,$item3)
    {
    	$this->db->select('*');
      	$this->db->from($table);
      	if(!empty($item1))
    ...
    public function list_common_1($table)
    {
    	 $this->db->select('*');
      $this->db->from($table);
	   $this->db->where('flag !=', '2');
    ...
    public function count_call_abort($table,$where,$id)
    {
    ...
    public function count_call_status_not($table,$where,$id)
    {
    	$this->db->select('*');
      $this->db->from($table);
	  $this->db->where('flag !=', '2');
  ...
  public function list_common_for_call($table)
  {
  		$this->db->select('*');
      $this->db->from($table);
    ...
    public function check_hierarchy($parent_id, $title)
  {
    ...
    public function list_common_join($id)
    {
    		$this->db->select('master_admin.*, caller_admin.*');
			$this->db->from('master_admin');
			$this->db->join('caller_admin', 'master_admin.admin_user_id = caller_admin.caller_admin_id', 'left');
			$this->db->where('caller_admin.caller_admin_id', $id);
			$query = $this->db->get();
			$result = $query->result_array();
			return $result;
    }
    ...
    public function list_common_where4($table, $where, $id,$where1,$mobile)
    {
      $this->db->select('*');
      $this->db->where($where, $id);
      $this->db->where($where1, $mobile);
      $this->db->order_by($where, 'asc');
      $this->db->from($table);
      $query = $this->db->get();
      $result = $query->num_rows();
